...
function parseSecurityDecisions(content, section) {
    // Find the section
...
export async function handleGetSecurityContext(topic) {
    ...
    let content = getCached(SECURITY_DECISIONS_FILE);
    if (!content) {
        try {
    ...
    if (topic) {
        ...
        decisions = decisions.filter(d => d.title.toLowerCase().includes(lower) ||
