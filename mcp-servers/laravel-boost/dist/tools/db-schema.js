...
function parseMigrationsForTables() {
...
function parseMigrationForTable(tableName) {
    const migrationsDir = resolve(BACKEND_PATH, 'database/migrations');
    if (!existsSync(migrationsDir))
        return null;
    const columns = [];
    try {
        ...
        for (const file of files) {
            ...
            if (content.includes('$table->softDeletes()')) {
                if (!columns.find(c => c.name === 'deleted_at'))
                    columns.push({ name: 'deleted_at', type: 'timestamp_nullable' });
            }
        }
    }
    catch {
...
export async function handleDbSchema(table) {
