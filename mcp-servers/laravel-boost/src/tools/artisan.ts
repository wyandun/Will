...
function isAllowed(command: string): boolean {
  ...
  for (const blocked of ARTISAN_BLOCKLIST) {
    if (base === blocked || command.startsWith(blocked + ' ')) return false;
  }
...
export async function handleArtisanCommand(command: string): Promise<string> {
  ...
  if (!isAllowed(cmd)) {
    return responseToText(
      buildError(
        `Command '${cmd}' is not in the allowlist. Allowed: ${ARTISAN_ALLOWLIST.join(', ')}`,
        { startMs }
      )
    );
  }
