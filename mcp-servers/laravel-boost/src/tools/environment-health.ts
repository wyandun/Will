...
import { execInDocker, execLocal } from '../utils/exec.js';
...
interface HealthResult {
  ...
  postgres?: { healthy: boolean; detail: string };
  redis?: { healthy: boolean; detail: string };
  queue?: { failedJobs: number; detail: string };
  api?: { healthy: boolean; statusCode?: number; detail: string };
  overall: 'healthy' | 'degraded' | 'down';
}

function checkContainers(): ContainerStatus[] {
...
function checkPostgres(): { healthy: boolean; detail: string } {
...
function checkRedis(): { healthy: boolean; detail: string } {
...
function checkQueue(): { failedJobs: number; detail: string } {
...
function checkApi(): { healthy: boolean; statusCode?: number; detail: string } {
  try {
    const out = execSync(
      'curl -s -o /dev/null -w "%{http_code}" http://localhost/api/v1/ping',
      { timeout: 5000, encoding: 'utf-8' }
    ).trim();
...
function calcOverall(result: HealthResult): 'healthy' | 'degraded' | 'down' {
  ...
  if (result.postgres && !result.postgres.healthy) return 'down';
  if (result.redis && !result.redis.healthy) issues.push(true);
  if (result.api && !result.api.healthy) issues.push(true);
  if (result.queue && result.queue.failedJobs > 0) issues.push(true);

  return issues.some(Boolean) ? 'degraded' : 'healthy';
}

export async function handleEnvironmentHealth(check?: string): Promise<string> {
  const startMs = Date.now();
  const c = check ?? 'all';
  const result: HealthResult = { overall: 'healthy' };
