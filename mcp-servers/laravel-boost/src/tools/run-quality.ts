...
export async function handleRunPint(fix?: boolean, path?: string): Promise<string> {
...
export async function handleRunPhpstan(path?: string): Promise<string> {
  const startMs = Date.now();
