...
export async function handleDescribeModel(model: string): Promise<string> {
  ...
  // File-parsed data
  let fileParsed: ReturnType<typeof parseModel> | null = null;
  try {
    let content = getCached<string>(filePath);
    if (!content) {
