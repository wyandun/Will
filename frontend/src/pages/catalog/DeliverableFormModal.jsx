      ...
      <div className="relative z-50 w-full max-w-lg mx-4 bg-white rounded-2xl shadow-xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col flex-1 overflow-hidden">
          <div className="px-6 py-5 space-y-4 overflow-y-auto">
            ...
            {/* Parent service — chip selector */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                {t('catalog.form.phase')} <span className="text-red-500">*</span>
              </label>
              <div className={`flex flex-wrap gap-2 rounded-lg border p-3 ${errors.parent_id ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-slate-50'}`}>
                {services.length === 0 && (
                  <p className="text-sm text-slate-400">{t('catalog.no_services')}</p>
                )}
                {services.map((s) => {
                  const selected = form.parent_id === String(s.id);
                  return (
                    <button
                      key={s.id}
                      type="button"
                      disabled={isSubmitting}
                      onClick={() => {
                        setForm((prev) => ({ ...prev, parent_id: String(s.id) }));
