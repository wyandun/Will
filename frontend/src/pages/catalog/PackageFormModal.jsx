...
/**
 * Modal to create or edit a package (bundle).
 * A package groups one or more services.
 */
export default function PackageFormModal({ bundle, services, onClose, onSave }) {
  const { t, i18n } = useTranslation('common');
  const isEditing = bundle !== null && bundle !== undefined;
  ...
  useEffect(() => {
    if (isEditing) {
      const currentServiceIds = Array.isArray(bundle.children)
        ? bundle.children.map((c) => c.id)
        : [];
      setForm({
  ...
  async function handleSubmit(e) {
    ...
    try {
      await onSave(payload, isEditing ? bundle.id : undefined);
    } catch (error) {
      ...
      <div className="relative z-50 w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="text-base font-semibold text-slate-800">
            {isEditing
              ? t('catalog.package.edit_title')
              : t('catalog.package.new_title')}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col flex-1 overflow-hidden">
          <div className="px-6 py-5 space-y-4 overflow-y-auto">
            ...
            <div>
              <label htmlFor="bun-desc" className="block text-sm font-medium text-slate-700 mb-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {s.service_type && (
                          <p className="text-xs text-slate-500 capitalize">
                            {t(`catalog.service_type.${s.service_type}`, {
